...
- 리더 없는 설정에서는 장애 복구가 불필요함.
- 단, 복제 서버가 죽어 있는 동안 데이터는 업데이트가 안되어 있으므로, 여러 서버에 클라이언트가 병렬로 요청을 보내고 버전 숫자를 사용해 어떤 값이 최신인지 판단함.

### 5.8.2 읽기 복구와 안티 엔트로피

- 읽기 복구: 클라이언트는 요청을 병렬로 보내고 가장 최신 버전의 데이터 가져옴. 읽기가 빈번한 작업에 유용

- 안티 엔트로피: 백그라운드 프로세스로 복제 서버간 데이터 차이를 찾아 지속적으로 하나의 복제 서버에서 다른 복제 서버로 이전.

### 5.8.3 읽기와 쓰기를 위한 정족 수

- n개의 노드가 있다면, 이 중 몇 개 이상의 노드에 쓰기 또는 읽기가 성공해야 읽기/쓰기 프로세스가 확정되는가?

#### 5.8.3.1 정족수 일관성의 한계

- 느슨한 정족수를 허용하는 경우

- 두 개의 쓰기가 동시에 발생하는 경우

- 쓰기와 읽기가 동시에 발생하는 경우

- 쓰기가 일부 노드에서는 성공했지만, 일부에선 실패해 전체 성공한 노드가 w 복제 노드보다 적은 경우

- 새 값을 전달하는 노드가 고장나서 예전 값을 가진 다른 복제 노드에서 데이터 복원 했는데 마찬가지로 복제 서버 수가 w 노드 보다 적은 경우

- 모든 과정이 올바르게 동작했으나, 시점 문제로 엣지 케이스 존재 가능.

### 5.8.4 모니터링

- 운영 관점에서 DB가 최신 데이터를 반환하는지 보는 것은 중요함.

- 복제가 뒤처지면 빠른 디버깅을 위해 모니터링 필수

  - POWA Demo : https://demo-powa.anayrat.info/server/1/database/tpc/overview/
  - POWA Github : https://github.com/powa-team/powa

### 5.8.5 느슨한 정족 수와 암시된 핸드오프

- 적절하게 설정된 정족 수는 장애 복구 없이 개별 노드 장애를 용인함.

- 하지만 지금까지 설명한 대로면 정족 수는 내결함성이 없음.

- 네트워크 중단으로 클라이언트와 노드 사이의 연결이 쉽게 끊어질 수 있음.

  - 예를 들어, 대규모 네트워크 장애로 일부 노드들만 운영이 된다면?
    - 트레이드 오프 직면
      - w나 r 노드 정족수를 충족하지 않는 모든 요청에 에러를 반환할 것인가
      - 아니면 일단 쓰기를 받고 보통 저장되는 n개 노드에 속하지 않는 노드에 기록할 것인가? -> 이걸 느슨한 정족수라 부름
      - 느슨한 정족수는 가용성을 높이는데 유용함.

### 5.8.6 다중 데이터 센터 운영

- 리더 없는 복제도 동시 쓰기 충돌, 네트워크 중단, 지연 시간 급증을 허용하므로 다중 데이터 센터 운영에 적합하다.

### 5.8.7 동시 쓰기 감지

- 다중 리더 복제 때와 마찬가지로 (**5.7.1 쓰기 충돌 다루기** 참고) 처리함.

### 5.8.8 최종 쓰기 승리 (동시 쓰기 버리기)

- 각 복제본이 갖고 있는 예전 값을 버리고 최신 값 사용하기.

- "최신"의 기준은 사용자에 의해 정의될 수 있음. 예를 들어, 타임 스탬프를 가장 최신 값으로 선택하고 싶다면 가장 최근 데이터를 덮어씌우는 형태.

- 이를 최종 쓰기 승리라고 함 (LWW, Last Writing Winner)

### 5.8.9 "이전 발생" 관계와 동시성

- 두 작업이 "동시에" 쓰였다는 것의 기준?

- 작업 B가 어떤 식으로든 작업 A를 기반으로 수행된다면, 이전 발생 (Happens-Before)라고 함.

- 한 작업이 다른 작업 이전에 발생했는지 규명하는 것이 동시성을 정의하는 핵심.

- 만약 작업 A가 작업 B보다 먼저 발생하지 않았다면 (즉, 두 작업 모두 서로에 대해 모른다면) 단순히 동시 작업이라함.

  - 동시성을 규명할 때, 시간은 크게 중요하지 않음.

### 5.8.10 이전 발생 관계 파악하기

- 앞에서 버전 번호가 최신인지 규명하는 내용이 있는데, 일맥상통함.

- 쓰기가 이전 읽기의 버전 번호를 알고 있으면 쓰기 수행 전에 이전 상태 알 수 있음.

- 버전 번호가 없는 쓰기는 다른 쓰기와 동시 수행된 것으로 아무것도 덮어쓰지 않음.

### 5.9.11 동시에 쓴 값 병합

- 동시 쓰기 값을 형제 값 (Sibling)이라함.

- 앞에 나온 자동 충돌 해소 알고리즘이 한 가지 해결책
