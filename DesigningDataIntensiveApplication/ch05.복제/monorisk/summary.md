...
## 다중 리더 복제
단리 리더를 사용한 복제는 일반적이며 단순하나, 모든 쓰기 연산이 하나의 리더를 거쳐야 하는 문제가 있다.
이를 완화하기 위해 쓰기를 허용하는 리더를 추가하는 방법이 있다.
하지만 이런 방법들은 쓰기 충돌 처리 등의 많은 추가 필요사항을 동반한다.
...
### 쓰기 충돌 다루기
**다중 리더 복제의 가장 큰 문제는 쓰기 충돌이 발생한다는 것이다.**
어떤 충돌은 명백하고 감지가 간단하다. 하지만 어떤 충돌은 감지조차 어렵다.

#### 동기 대 비동기 충돌 감지
동기 충돌 감지는 이론적으로는 가능하나 다중 리더를 쓰는 이점을 모두 잃기 때문에 사용하지 않는다.

#### 충돌 회피
가장 간단한 전략은 충돌을 회피하는 것이다.
특정 레코드의 모든 쓰기가 동일한 리더를 거치도록 하면 충돌은 발생하지 않는다.
하지만 특정 리더가 있는 데이터센터에 장애가 발생하거나, 지리적 위치가 변경 되어 데이터센터가 변경 되면 충돌이 다시 발생한다.

#### 일관된 상태 수렴
다중 리더 모델에서는 순서의 개념이 명확하지 않다.
모든 복제 계획은 모든 복제 서버가 최종적 일관성을 갖는 것을 목표로 한다.
이를 위해 수렴 방식으로 충돌을 해소해야 한다.

이를 위한 방법은 다양하다.
* 각 쓰기에 고유 ID를 부여하고, 가장 높은 ID를 가진 쓰기를 고른다.
* 타임스탬프를 사용하는 경우는 최종 쓰기 승리(Last Write Wins, LWW)라고 한다.
* 각 복제 서버에 고유 ID를 부여하고 높은 숫자의 복제 서버에서 생긴 쓰기가 낮은 숫자의 복제 서버에서 생긴 쓰기보다 항상 우선적으로 적용되게 된다.
* 어떻게든 값을 병합한다. (ex: 값을 사전순으로 정렬하고 연결한다.)
* 명시적 데이터 구조에 충돌을 기록해 모든 정보를 보존한다. 나중에 충돌을 해소하는 애플리케이션 코드를 작성한다

#### 사용자 정의 충돌 해소 로직
사용자 정의 충돌 해소 로직의 동작 시점은 크게 두 가지가 있다.

##### 1. 쓰기 수행 중
복제된 변경 사항 로그에서 데이터베이스 시스템이 충돌을 감지하자 마자 충돌 핸들러를 호출하는 방식.
일반적으로 사용자에게 충돌 내용을 표시하지 않으며, 백그라운드에서 빠르게 수행 되어야 한다.

##### 2. 읽기 수행 중
충돌을 감지하면 모든 충돌 쓰기를 저장한다. 다음 번 데이터를 읽을 때 이런 여러 버전의 데이터가 애플리케이션에 반환된다.
애플리케이션에게 충돌 해소를 요청하거나 자동으로 해소할 수 있다. 해소된 결과는 DB에 기록된다.

### 다중 리더 복제 토폴로지
복제 토폴로지는 쓰기를 한 노드에서 다른 노드로 전파하는 방법을 의미한다.

##### 원형 토폴로지
각 노드가 하나의 노드로부터 쓰기를 받아 처리하고, 자신의 쓰기를 담아 다른 노드에게 전달한다.
무한 루프를 방지하기 위해 쓰기 모든 노드는 고유 식별자가 있고, 쓰기 요청에 노드의 식별자를 태깅하여 이미 처리한 사실이 있는지 확인한다.

##### 별 모양 토폴로지
원형과 같으나, 지정된 한 노드가 전체 다른 노드에게 쓰기 전달을 수행한다.

##### 전체 연결 토폴로지
클러스터내 일부 노드의 장애에 대응하기 위해 전체 연결을 고려할 수 있다.
단일장애점을 회피할 수 있으나, 노드간 네트워크 상황등에 의해 메시지 추월이 발생할 수 있다.
(메시지 추월이 발생하면 최종적 일관성이 실패한다.)

이런 이벤트를 올바르게 정렬하기 위해 버전 벡터라는 하는 기법을 사용할 수 있다.
