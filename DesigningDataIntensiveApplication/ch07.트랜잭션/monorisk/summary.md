...
* 스큐(SKEW): 여기에서는 시간적인 이상 현상 정도로 사용됨
...
## 직렬성
직렬성 격리는 보통 가장 강력한 수준의 격리로 여겨지고 데이터베이스가 발생할 수 있는 모든 경쟁 조건을 막아준다.
하지만 직렬성의 구현 방식마다 장단점이 존재한다. 이 방법과 트레이드오프를 이해해야 한다.

### 실제적인 직렬 실행
동시성 문제를 피하는 가장 간단한 방법은 동시성을 완전히 제거하는 것이다.
뻔한 해결책이지만, 최근에 들어서야 두 가지 발전으로 인해 단일 스레드에서도 높은 성능을 낼 수 있다고 생각하게 되었다.
1. 램 가격이 저렴해져서 활성화된 데이터셋 전체를 메모리에 유지할 수 있게 되었다. 이로인해 트랜잭션이 디스크에 접근할 때보다 매우 빨라졌다.
2. 데이터베이스 설계자들은 OLTP 트랜잭션이 보통 짧고 실행하는 읽기와 쓰기의 개수가 적다는것을 깨달았다. 반대로 오래 실행되는 분석 질의는 전형적으로 읽기 전용이라서 직렬 실행 루프 밖에서 일관된 스냅숏을 사용해 실행할 수 있다.

#### 트랜잭션을 스토어드 프로시저 안에 캡슐화하기
데이터베이스 초창기에는 데이터베이스 트랜잭션이 사용자 활동의 전체 흐름을 포함할 수 있고 이것들을 원자적 처리 하려는 의도가 있었다. 하지만 사용자 입력을 기다리는 것은 매우 긴 시간이 필요했고 트랜잭션을 효율적으로 처리할 수 없었다.
이때문에 많은 OLTP 애플리케이션은 사용자의 응답을 기다리는 것 보단, 흐름을 분할하여 응답을 기다리지 않고 트랜잭션을 처리할 수 있도록 했다. 이로인해 서버/클라이언트 스타일의 사용법이 보편화됐고, 이러한 상호 작용 방식은 네트워크 통신에 많은 시간을 소비한다.

이런 상황 때문에 단일 스레드에서 여러 트랜잭션을 순차적으로 처리하는 시스템들은 이런 처리를 지원하지 않고, 스토어드 프로시저 형태로 제공한다.
애플리케이션은 트랜잭션 코드 전체를 스토어드 프로시저 형태로 데이터베이스에 미리 제출하고 데이터베이스는 이를 수행한다.

하지만 다음의 단점들이 있다.
* 데이터베이스 벤더마다 제각각 스토어드 프로시저용 언어가 있다. 이 언어들은 언어의 발전을 따라잡지 못해 조잡하거나 생태계가 빈약한 경우들이 많다.
* 데이터베이스에서 실행 되는 코드 고나리가 어렵다.
* 데이터베이스는 애플리케이션 서버보다 성능에 훨씬 민감한 경우가 많은데, 잘못 작성된 스토어드 프로시저 코드가 데이터베이스에 영향을 줄 수 있다.

#### 파티셔닝
모든 트랜잭션을 순차적으로 실행하면 동시성 제어는 훨씬 간단하지만 데이터베이스의 트랜잭션 처리량이 단일 장비에 있는 단일 CPU 코어의 속도로 제한된다.
데이터 파티셔닝을 통해 여러 CPU 코어와 노드로 선형적으로 확장할 수 있다.

하지만 여러 노드에 걸친 트랜잭션이 있다면 직렬성 보장을 위해 여러 파티션에 걸쳐 잠금 획득 등의 코디네이션이 필요하다.
이는 코스트의 문제로 이어진다.
...
### 2단계 잠금
**2PL은 2PC가 아니다.**
2단계 잠금은 공유/독점 모드 잠금을 이용하여 구현 된다.
1. 트랜잭션이 객체를 읽기 원한다면 공유 모드로 잠금을 획득해야 한다. 동시에 여러 트랜잭션이 공유 모드로 잠금 획득할 수 있지만, 만약 그 객체에 독점 모드로 잠금을 획득한 트랜잭션이 있으면 이 트랜잭션이 완료될 때까지 기다려야 한다.
2. 트랜잭션이 객체 쓰기를 원하면 독점 모드로 잠금을 획득해야 한다. 다른 어떤 트랜잭션도 동시에 잠금을 획득할 수 없으므로 그 객체에 잠금이 존재한다면 트랜잭션은 대기해야 한다.
3. 트랜잭션이 객체를 읽다가 쓰기를 실행할 때는 공유 잠금을 독점 잠금으로 업그레이드 해야 한다. 업그레이드는 독점 잠금을 직접 획득할 때와 똑같이 동작한다.
4. 트랜잭션이 잠금을 획득한 후에는 트랜잭션이 종료 될 때까지 잠금을 갖고 있어야 한다. 그래서 2단계라는 이름으로 불린다.

2단계 잠금의 쓰기 트랜잭션은 다른 쓰기 뿐만 아니라 다른 읽기 또한 막는데, 이것이 스냅숏 격리와 2단계 잠금의 가장 큰 차이점이다.

교착상태가 쉽게 발생할 수 있으며, 데이터베잇는 트랜잭션 사이의 교착 상태를 자동으로 감지하고 트랜잭션 중 하나를 어보트시켜서 다른 트랜잭션들이 진행할 수 있게 한다.

#### 2단계 잠금의 성능
2단계 잠금의 가장 큰 약점은 성능이다.
잠금의 획득/해제 오버헤드도 문제이지만, 동시성이 떨어지는 것이 가장 큰 문제이다.

#### 서술 잠금
서술 잠금은 공유/독점 잠금과 비슷하게 동작하지만 특정 객체에 속하지 않고 조건에 해당하는 모든 객체에 속한다.
서술 잠금이 접근을 제한하는 방법은 다음과 같다.
* 트랜잭션 A가 어떤 조건에 부합하는 객체를 읽기 원한다면 질의 조건에 대한 공유 모두 서술 잠금을 획득해야 한다. 다른 트랜잭션 B가 그 조건에 부합하는 어떤 객체에 독점 잠금을 가지고 있으면 A는 질의를 실행하도록 허용되기 전에 B가 잠금을 해제하기를 기다려야 한다.
...
핵심 아이디어는 서술 잠금은 데이터베이스에 아직 존재하지 않지만 미래에 추가될 수 잇는 객체에도 적용할 수 있다는 것이다. 2단계 잠금이 서술 잠금을 포함하여 데이터베이스에서 모든 형태의 쓰기 스큐와 다른 경쟁 조건을 막을 수 있어서 격리 수준이 직렬성 격리가 된다.

#### 색인 범위 잠금
진행 중인 트랜잭션들이 획득한 잠금이 많으면 조건에 부합하는 잠금을 확인하는데 시간이 오래 걸린다. 이때문에 2PL을 지원하는 대부분의 데이터베이스는 실제로는 색인 범위 잠금을 구현한다.
이것은 서술 잠금을 간략하게 근사한것이다.
